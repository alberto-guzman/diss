...
My proposed simulation design is informed by earlier propensity score estimation simulations conducted by Setoguchi et al. (2008), Lee et al. (2009), and Cannas and Arpino (2019). Using a Monte Carlo simulation design, I compare the performance of various propensity score estimation methods against the traditional logistic regression
and my proposed DNN-based approach in a high-dimensional context. I assess the performance by how well each method captures a simulated ATE and balances the
observed covariates. Specifically, I access the performance of popular machine learning techniques used in the propensity score estimation literature, including classification and regression tree (CART), bagged CART, random forest, and single-layer neural network. In this section, I describe the simulation design for this study, including the data generation process and the scenarios that I consider.

#### Covariates

I generated three covariate levels that varied in dimensionality ($p = 20, 100, 200$) . The covariates were generated from random pulls from three distributions; a normal distribution with a mean of 0 and SD of 1, a uniform distribution with a range from 0 to 1, and a Bernoulli distribution with a probability of success set to 0.5. For each covariate level half of the covariates were generated from the normal distribution, a quarter from the uniform distribution and the remaining quarter from the Bernoulli distribution. The majority of the propensity score simulation literature generate uncorrelated covariates (CITE), however, this is not representative of the correlated data typically found in administrative datasets. Therefore, for half of the covariates I induced a correlation structure among the covariates by using the inverse CDF (cumulative distribution function) method (CITE).
...
#### Population Propensity Score Models

The population propensity score models - treatment assignment models - were generated using a logistic regression:

$$
Pr(Z = 1) = (1 + exp(-(\beta_0 + \sum_{i=1}^{p} \beta_iX_i)))^{-1}
$$

where $X_i$ represents $p$ covariates with their associated coefficients ($B_i$) . The binary treatment assignment variable was generated by comparing and indiduals true propensity score to a random draw from a uniform distribution bounded between 0 and 1. If an individuals propenisty score was higher than the random draw from the uniform distribution the indidual was assigned to treatment, Zi was assinged a 1 , othewise to 0 . Using this approach the probability of assignment to treatment should be close to 0.5.

Next

Individuals outcomes were generated as a linear combination of covariates, as follows:

$$
Y(Z) = \alpha^Z + \sum_{i=1}^{p} \pi_iX_i; Z = 0,1
$$

where $\alpha^Z$ represented the coefficient and intercept for $\alpha^0$ and $\alpha^1$ which are set such that the population treatment effect is 0.3. Which is in line with the typical small treatment effects we observe in education research (CITE). The individual outomes ($Y$) were

given education intervnetion have realteli small effects, average effect is .28 DF

Treatment assignment was generated using a logistic regression:

I will generate covariates that vary in dimensionality and sample size, and are of various scales (e.g., continous, categorical) to reflect the diverse datasets encountered by psychological and educational researchers. I generate three dimension of covariates (p = 20; 100; 200) and two sample sizes (300,000). The low-dimension setting was chosen since its around the median number of covariates used in a typical applied propensity score analysis in educaiton research (CITE).

Covariates were generated from random pull from a normal distribvution (50%) with mean zero and standard deviation of one, uniform distribution with a range from 0 to 1 of whole integers (25%), and a bernoulli distribution with probalby of success p = .50 (25%). I tranform half of the normally distribute varibles into ordinal variablers by creating 5 equal interval cuts, this mimics a 5-point likert scales commonly used in educational research. Correlation was induced

#### Propensity Score Model

We generated a treatment and outcome model with varying levels of complexity including combinations of base model, linearity, and non-additivity:

A propensity score model for a binary treatment was generate using a logistic model:

$$
Pr(Z = 1) = (1 + exp(-(\beta_0 + \sum_{i=1}^{p} \beta_iX_i)))^{-1}
$$

Our outcome model was generated as a linear combination of covariates, as followes:

$$
Y(Z) = \alpha^Z + \sum_{i=1}^{p} \pi_iX_i; Z = 0,1
$$

Whe varied our propensity score model and out outcome model by adding interaction (non-additivity) and higher order term (non-linearitys.
...
$$
Y^{b}(Z) = g^a(X) + \alpha_iX_i^2; Z = 0,1
$$
...
$$
Y^{c}(Z) = g^a(X) + \alpha_iX_iX_{i+1}; Z = 0,1
$$
...
$$
Y^{d}(Z) = g^a(X) + \alpha_iX_i^2 + \alpha_iX_iX_{i+1}; Z = 0,1
$$
...
We test a total of 16 simulation scenarion with three levels of covariates. For a total of 48 simulation conditions.

logistic regression, classification and regression tree (CART), bagged CART, random forest, and single-layer neural network, deep neural network

method , parameters and which R package that was used.
...
Estimatint the treamtnet effects

IPTW exstimate, using a regression model and talka bout the IPTW weights, should I trim weights?

Performance metrics

I will evaluate the performance of the various propensity score estimation methos though the folowing measures:

average standrized absoluse mean distance; measure of covariate balance
