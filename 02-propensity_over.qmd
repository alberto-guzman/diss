...
# Load sim results
res <- readRDS("~/Projects/inProgress/2018_propensity_neuralnet_paper/data/res_all.rds")

# Generate sim_sum results overall
simsum_res_all <- simsum(data = res, estvarname = "ATE", true = 0.3, se = "ATE_se", methodvar = "method", by = c("p"), ref = "logit")
```
...
res %>%
  ...
  geom_pointrange(aes(ymin = bias - bias_mcse, ymax = bias + bias_mcse), 
                position = position_dodge(0.9), size = 0.1, alpha = 0.3) +
  geom_hline(yintercept = -.2, color = "white", size = 1) 
```
...
res |> 
  ...
  theme_Publication() +
  scale_fill_Publication() +
  geom_pointrange(aes(ymin = rel_bias - rel_bias_mcse, ymax = rel_bias + rel_bias_mcse), position = position_dodge(0.9), size = 0.1, alpha = 0.3) 
