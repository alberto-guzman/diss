...
# Generate upper and lower CI 95%
res <- res |> 
  mutate(lower = ATE - (1.96 * ATE_se), upper = ATE + (1.96 * ATE_se), true_ATE = 0.3)
```
...
# Calculating relative bias
res %>%
  group_by(p, method) %>%
  do(calc_absolute(., estimates = ATE, true_para = true_ATE)) %>%
  ...
  geom_pointrange(aes(ymin = bias - bias_mcse, ymax = bias + bias_mcse), 
                position = position_dodge(0.9), size = 0.1, alpha = 0.5)
