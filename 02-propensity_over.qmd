...
$X_i$ represents $p$ covariates and their associated coefficients ($\beta$). The output, $Pr(Z = 1)$ is the probability of assignment to the treatment condition (i.e., the propensity score). The values for the $(\beta_X)$ coeffecients for the population treamtnet models were based on the evalution of the common application such that the intercept $0.25$, and the remaining $\beta_X$ coefecients were given a random value between -0.4 and 0.4.
...
$$
PS^{A}(X) = (1 + exp(-(\beta_0 + \sum_{i=1}^{p} \beta_X)))^{-1}
$$ {#eq-psA}
