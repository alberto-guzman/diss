...
In this section I outline the results of the simulation study in three major sections. First, I present restulst in terms of bias in the estimation of the ATE, then I analyze the variablity in the ATE estimation, followed by an analyzese of covariate balance. Finally, I end this section discussion an analyzzis of the IPTW weights and statistical power. In each section I organize the results by complexity in the treatment and outcome models, begining with base and then complex. I also look at dimensionality
...
#### Diversion Treatment and Outcome Models

Figure X anything off the dianol in the panels signify diversions from the treatmetn and outcome model. For example, the lower left panel are the results for when the treatment model is complex and the outcome model only has base terms. Overall i find that when complexities are in the outcome model affect bias performance compared to complexities in the treatment model. I find that the my dnn approach produces the least biased estimates in the scenario where complexities are indlued in the outcome model but not in the treatment model. Overall, bias perfromance appear to be sensitive to mispecfication in the outcome model, or when miscpeficaiton are present in both outcome and treatment model. 
