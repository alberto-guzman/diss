...
I begin by summarize the bias in the average treatment effect (ATE) across all population data generating models (Base(T), Complex(T), Base(Y), Complex(Y)) to gain a sense of the overall performance of the methods across conditions. @fig-bias presents the results for the bias of the ATE by method and covariate dimensionality (p = 20, 100, 200). The "true" ATE is set to 0.3, so a bias of 0 indicates that the method has accurately recovered the true ATE without any bias. If the bar is above 0, the method overestimates the true ATE, and if it is below 0, it underestimates the ATE.
...
In addition to raw bias, I also evaluated the absolute percent bias, which is presented in @fig-abs-bias. An absolute percent bias of 0 indicates no bias in the estimated ATE.
