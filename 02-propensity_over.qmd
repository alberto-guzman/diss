...
The strong ignorability assumption states that the potential outcome of an individual is conditionally independent of treatment assignment if we successfully conditioned on all covariates related to treatment, $Y_1, Y_0 \perp Z|\boldsymbol{X}$. In experimental studies, this assumption is satisfied by the act of randomization, since randomization ensures that treatment assignment is independent of an individual's outcome [@rubin2005causal; @Rosenbaum.2022].However, in observational studies, this assumption holds only if all covariates related to treatment assignment are included in the propensity score model [@Rosenbaum.1981; @Rosenbaum.1984]. In practice, this is unlikely and therefore sensitivity analysis is necessary to assess the influence of unmeasured confounding (CITE).

Finally, the region of common support assumption states that there must be sufficient overlap in the estimated propensity score distribution between the treatment and control groups [@Rosenbaum.2022]. This is important to ensure that we can find suitable matches for treated students. Adequacy of the common support can be assessed by plotting the distribution of the propensity scores in both the treatment and control groups and assessing if the distributions have sufficient overlap. This can be done by visually examining the distribution plots for treatment and control groups and determining if there is enough overlap between the two groups [@Rosenbaum.2022].

If these assumptions are met, it is possible to estimate the causal effects from observational data. However, many of these assumptions are untestable [@Stuart.2010; @ho2007matching; @Lee.2010]. For instance, in observational data, the underlying treatment assignment mechanism is usually unknown, making it difficult to determine if all covariates related to treatment assignment are included in the propensity score model [@Stuart.2010]. As a result, propensity score analysis should be considered as a method to reduce, but not eliminate, the bias between treatment and outcome in observational studies. It is essential to carefully consider which covariates are included in the propensity score model and to perform sensitivity analysis to assess the impact of unmeasured confounding (CITE).

#### Covariate Selection

A reasonable approach to selecting covariates in propensity score analysis is to choose those that are theoretically sound and related to treatment assignment and the outcome, also known as confounders [@Stuart.2010; @Bai.2011; @Pan.2018qcl]. However, there is a debate in the literature as to which covariates should be included \[Austin, 2011\]. Some simulation studies have found that including confounders or covariates that only affect the outcome leads to more precise treatment effect estimates [@Austin.2007]. Additionally, other research has shown that only including covariates that affect treatment assignment results in increased variance in the estimated treatment effect with no reduction in bias [@Brookhard.2006].

In practice, it can be challenging to determine which covariates are confounders or related to just the treatment or the outcome. To guard against the penalties of excluding potential confounders, the literature suggests using a "kitchen sink" approach to variable selection [@Pirracchio.2015; @Shortreed.2017lu8]. Additionally, it is vital to ensure that the propensity score model captures the correct functional form of the covariates, including all relevant interactions and non-linear terms [@Rosenbaum.2010; @Pan.2018qcl]. If the propensity score model is misspecified, it can result in biased treatment effect estimates [@Rosenbaum.1984; @Rosenbaum.1981; @Guo.2009].

In order to achieve a balance among observed characteristics between treated and untreated students, iterating through various model specifications that add interactions or non-linear terms is recommended [@murnane2010methods; @Rosenbaum.2010]. However, this iterative process can become increasingly complex and time-consuming as the number of available covariates increases.
