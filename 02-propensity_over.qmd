...
Figure x showe the results for weights, note that by contructions all weights will have a value above 1. All machine learnig methods lead to acceptable weights below 2.5. However, in the high dimensional cases logisti regression is producing extreme weights.
...
Figure X show the results for power. In the low dimesnion case lgoistic regression has high power, correclty finding a statsitically significant treatment effect 100% of the time. As we introduce more covarite the logistic models power reduces. In the high dimensional settting particualry fores t and cart.
