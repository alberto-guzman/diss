...
df1 <- readRDS("~/Projects/inProgress/2018_propensity_neuralnet_paper/data/sim_results_n10000_r100_NP_e.rds")
...
Given the high-dimensional data generated in my simulation and computationall complextity of the methods, I used the University of Pittsburgh Center for Research Computing cluster to run the simulation on high-memory nodes. All simulations were perfromed in R version 4.1.0 and Python 3.7, and Keras version and Tensorflow ...

## Results

This section presents the results of the simulation study in three main parts. Firstly, I will show the results regarding the bias in the ATE estimation. Secondly, I will examine the variability in the ATE estimation. Thirdly, I will analyze the covariate balance. Finally, I will conclude this section with a discussion of the IPTW weights and statistical power. The results are organized based on the complexity of the treatment and outcome models, starting with the basic models and then moving on to the complex models. Additionally, the results are organized based on the dimensionality of the data.

### Bias of ATE Estimation

My primary goal with this simulation was to evalute which methods produced the most unbiased estimate in terms of the ATE in high dimensional setting. Given that the main goal with any type of quasi-experimental method is to recover the "true" population treatment effect. To begig I summarize the simulation results across all condtiions (Base(T),Complex(T),Base(Y), Complex(Y)) in order to get a sense of overall performance across condtions. I focus on comparing performance of each method across covariate dimensionalities. @fig-abs-bias, I present the absolute percent bias by method and across covariate conditoins. Overall, I find that in the low-dimensional case, 20 covarites, logistice regresison produces the least amount of bias, although the DNN and NN approaches are fairly in line and all other approaches all produceing less that 5% bias (CITE). One expection is bagged cart produces a large a moutn of bias with 20 covariates. As I increase the number of covariates from 20 to 100 and 200 logistic regression starts to produce signifncalty worse than all other methods. With 100 covaraites I find that all non-paramatric methods perform realtively well with all having bias well below 5%. IN the 100 covariate sense the lowest bias estimates are produced by my DNN approach specifically DNN-2. With 200 covariates all machine learnign approach perfrom well although I do see an increase in bias across all methods. Forest produces the least biased estimates followed by bagged cart, nn-2 and dnn-3. Overall, these findings suggest that in the low dimensional setting logistic regresison produces the least biased estimated, while as dimenstionally increase logistic regression produces widely biased estimates compared to all other machine learning approaches. Of the machine learnig approaches my dnn approaches produce lower bias in 100 covariates while random forest performs better in the 200 covaraite case. Somethis about logist bias x times more than even blah blah.

```{r}
#| label: fig-abs-bias
#| fig-cap: "Absolute Bias (%) by methods and covariates"
...
#### Treatment and Outcome Models are Correctly Specified

Next I look bias performance of each method based on complexities in the treatment and outcome model. @fig-abs-bias-cond presents a paneled graph where the upper left panel indicates the condition in which both the treatment and outcome model where correclty specifice, meaning only main effects where included in the data generation process, and where then being estimated by main effects with each method. Overall I find that all methods across conditions performed well with bias being less than 5%, with the exception of bagged cart which produced extremely biased results with 20 covariates. Not suprising in the the 20 covariate case logistic regression produces the least biased estimates, however the neural network approaches are within the same area. As the number of covariates increase I find that all methods perform exceptionally well, with logistic and the neural network approaches bproducing the least biased estimates.

```{r}
#| label: fig-abs-bias-cond
#| fig-cap: "Absolute Bias (%) by methods and covariates and treatment and outcome model"
#| fig-width: 20
#| fig-height: 20
...
When the treatment and outcome models are incoreclty specified, meaning that quadratics and interactions are included in the data generating mechanisms, while each method is estimating the propensity score with only main effects. In the low dimenasional case all methods do exceptionally well with logistic regression producing the biased results, as with the base model bag cart produces the most amount of bias. With 100 covariates all non-parametric methods perform well below 5% bias with with dnn-3 and cart producing the least biased estiamtes. In the higher dimensional case all mahcine learning methods perform well, however dnn-2 and dnn-3 produced slighly higher estimates, the random forest techniques produced the least biased results.

#### Deviations from the Treatment and Outcome Models

In @fig-abs-bias-cond , deviations from the treatment and outcome model are indicated by points off the diagonal in the panels. The lower left panel displays the results when the treatment model is complex and the outcome model only has main effects. My findings show that when complexities are present in the outcome model, they tend to have a greater impact on bias performance compared to complexities in the treatment model. The DNN approach I propose produces the least biased estimates in scenarios where complexities are included in the outcome model but not in the treatment model. In conclusion, bias performance appears to be sensitive to mispecification in the outcome model or when mispecifications exist in both the outcome and treatment models.

### Variability and Mean Squared Error (MSE) of ATE Estimation

Although the ATE point estimates are crucial metrics, it is also important to consider the variability of the ATE. @fig-ate-var showcases a comparison of each method's performance across different dimensions, focusing on the standard error of the ATE, mean squared error, and 95% confidence interval coverage. The optimal method should not only have a low bias in the ATE point estimate but also exhibit minimal variability. As seen in the @fig-ate-var-1, in the low-covariate scenario of 20, all methods display relatively similar results in terms of standard error of the ATE. As I increase the number of covariates logistic regression produced larger standard errors, while th emachine learning methods produce far smaller standard errors. Across both 100 and 200 covariate scenarios random fores t and cart perform well leading to the smalles standard erorrs. My dnn aporach perform well. Overall, these findings suggest that in low-dimensional scenarios logistic regression produces the least biased estimate, while in high-dimensional scenarios, machine learning approaches perform better, with random forest and bagged cart having the smallest standard errors.

```{r}
#| label: fig-ate-var
#| fig-cap: ATE Variability
#| fig-subcap: 
#|   - "Standard Error (SE)"
...
The mean squared error (MSE) is an important metric that helps to evaluate both the bias and the spread of the ATE. It measures the average squared difference between the estimated values and the true values, where a lower MSE indicates a better fit between the estimated and true values. @fig-ate-var-2 presents the results of the simulation in terms of mean squared error. Just like the standard error results, in the low dimensional scenario with 20 covariates, all methods perform similarly. However, as the dimensionality increases, I find that the DNN-2 approach performs the best and leads to the lowest MSE value. This suggests that the DNN-2 approach provides a good fit between the estimated and true ATE values, even in high-dimensional data.

In the context of simulation studies, the coverage rate of the 95% confidence interval is a crucial metric to consider. It reflects the precision of the estimated ATE and the likelihood of capturing the true ATE within the interval. A low coverage rate implies a wide confidence interval, which means the estimated ATE is imprecise and uncertain. On the other hand, a high coverage rate indicates a narrow confidence interval, which means the estimated ATE is more precise. Evaluating the coverage rate of the 95% confidence interval provides valuable insights into the consistency and reliability of the estimation methods used. @fig-ate-var-3 In the low-dimensional setting, logistic regression yields a better coverage rate compared to other methods. However, with high numbers of covariates, bagging has the best coverage rate, followed by logistic regression and the neural network approaches. On the other hand, both CART and random forest have a relatively low coverage rate.

#### Deviations from the Treatment and Outcome Models

```{r}
#| label: fig-ate-var-cond
#| fig-cap: ATE Variability by Data Generating Models
#| fig-subcap: 
#|   - "Standard Error (SE)"
#|   - "Mean Squared Error (MSE)"
#|   - "95% CI Coverage"
...
gt(init_df) |>
  ...
  fmt_number(columns = c(Std_In_Bias, Prob_Treat), decimals = 3) |>
  cols_label(
