...
This section presents the results of my Monte Carlo simulation study in three main parts. First, I present results on bias in the average treatment effect (ATE) estimation. Then, I examine the variability in the ATE estimation, specifically by evaluating the standard error (SE), mean squared error (MSE), and 95% confidence interval coverage of the ATE. After that, I evaluate covariate balance through the average standardized absolute mean difference (ASAM). I conclude by presenting the results on mean IPTW weights and statistical power.

### Bias of ATE Estimation

My primary aim with this simulation was to evaluate which propensity score estimation method produced the least biased estimate of the average treatment effect (ATE) in a high-dimensional covariate setting. Since the goal of quasi-experimental studies is to employ a method that can accurately recover the "true" population treatment effect with minimal bias. However, in reality, it is challenging to quantify the amount of bias introduced by a specific method. Simulation studies provide a valuable tool for evaluating the performance of different methods in a controlled and systematic manner.

I begin by summarize the bias in the average treatment effect (ATE) across all population data generating models (Base(T), Complex(T), Base(Y), Complex(Y)) to gain a sense of the overall performance of the methods across conditions. Figure @fig-bias presents the results for the bias of the ATE by method and covariate dimensionality (p = 20, 100, 200). The "true" ATE is set to 0.3, so a bias of 0 indicates that the method has accurately recovered the true ATE without any bias. If the bar is above 0, the method overestimates the true ATE, and if it is below 0, it underestimates the ATE.

In the low-dimensional setting of 20 covariates, all methods recover the true ATE with little bias, except for bagged CART, which substantially underestimates the true ATE. When the covariates increase to 100, logistic regression begins to overestimate the true ATE, while all other machine learning approaches recover the ATE with less bias. In the highest covariate case, these results become more striking. Logistic regression severely overestimates the true ATE, while the machine learning methods recover the true ATE with significantly less bias. A notable exception is nn-1, which in the high covariate scenario severely underestimates the true ATE, with a larger magnitude than logistic regression, albeit in the opposite direction.
...
In addition to raw bias, I also evaluated the absolute percent bias, which is presented in Figure @fig-abs-bias. An absolute percent bias of 0 indicates no bias in the estimated ATE.

In the low-dimensional case of 20 covariates, logistic regression produces the least amount of absolute bias, although the DNN and NN approaches are fairly in line, and all other approaches produce less than 5% absolute bias (CITE). The exception is bagged CART, which produces a large amount of bias with 20 covariates (absolute bias = 11.3%). As the number of covariates increases from 20 to 100 and 200, logistic regression starts to produce significantly worse results than all other methods. With 100 covariates, all non-parametric methods perform relatively well, with all having bias well below 5%. In the 100 covariate scenario, the lowest bias estimates are produced by my DNN approach, specifically DNN-2, and nn-1 (0.43%, 0.21%, respectively). With 200 covariates, all machine learning approaches perform well, although I do see an increase in bias across all methods. Bagged CART produces the least biased estimate (absolute bias = 1.02%) followed by the DNN-2 approach (absolute bias = 1.33%), although the magnitude difference is relatively small. An interesting finding is that the nn-1 approach produces the most biased results, even above that found by logistic regression (23.5%, 5.18%, respectively).
...
#### Base Treatment and Outcome Population Models

Next, I evaluate the absolute bias performance of each method based on the complexities introduced into the treatment and outcome model. The data generating models "Base_T" and "Base_Y" include only main effects in the treatment and outcome model, while "Complex_T" and "Complex_Y" include quadratic and interaction terms among the covariates.

Figure \@fig-abs-bias-cond presents a paneled graph where the upper left panel indicates the condition in which both the treatment and outcome model include only main effects. Overall, I find that all methods across conditions perform well, with bias being less than 2.5%, with the exception of bagged CART, which produced extremely biased results with 20 covariates (absolute bias = 18.3%). In the 20 covariate case, logistic regression produces the least biased estimates (absolute bias = 0.01%) since both the data generating models and the propensity score estimation models include only main effects. As the number of covariates increases, I find that all methods perform exceptionally well, with my DNN-3 approach producing the least biased estimate (absolute bias = 0.11%).
...
#### Complex Treatment and Outcome Population Models

In contrast to the models where only main effects are included in the treatment and outcome population models, the lower right panel of Figure \@fig-abs-bias-cond presents the results when both the treatment and outcome population models include quadratic and interaction terms among the covariates. I find that in the low-dimensional case, the methods are relatively insensitive to the complexities in the population models as they all perform relatively well. However, as the number of covariates increases, we see a stark increase in bias compared to the base models. With 100 covariates, all methods perform well below 1.5% absolute bias, with bagged and nn-1 producing the least biased estimates (absolute bias = 0.08%, 0.13%, respectively). We start seeing more stark differences when we are in the higher covariate condition. Specifically, logistic and nn-1 produce the largest biased ATE estimates (absolute bias = 16.6%, 87.9%, respectively). The least biased estimates are produced by the DNN-2 approach (absolute bias = 2.67%). These findings suggest that DNN-2 generates the least biased ATE effects not only in comparison to the logistic model but all other widely used machine learning-based propensity score estimation methods.

#### Deviations from the Treatment and Outcome Models

In Figure \@fig-abs-bias-cond, deviations from the treatment and outcome model are indicated by points off the diagonal in the panels. The lower left panel displays the results when the treatment model is complex and the outcome model only has main effects, while the upper right panel displays the results when the outcome model is complex but the treatment model only has main effects.

In general, I find that when complexities are present in the outcome model, they tend to have a greater impact on bias performance compared to complexities in the treatment model. The DNN approach I propose produces the least biased estimates in scenarios where complexities are included in the outcome model but not in the treatment model.

In conclusion, the results suggest that bias performance is sensitive to complexities in the outcome model or when complexities exist in both the outcome and treatment models. The findings suggest that the DNN approach is a robust method for estimating the ATE in high-dimensional covariate settings, especially when complexities are present in the outcome model.

### ATE Estimation: SE, MSE, and 95% CI Coverage

Although bias in the ATE point estimates is crucial, it's also important to consider the sampling variability of the ATE. The optimal method should not only have low bias in the estimated ATE but also exhibit minimal variability through small standard errors (SE) and mean squared errors (MSE), and high 95% confidence interval coverage. These properties will lead to increased precision in the estimated ATE.

\@fig-ate-var compares the performance of each method in terms of standard error of the ATE, mean squared error, and 95% confidence interval coverage across different covariate conditions. In the low-covariate scenario of 20, all methods have small standard error. However, as the number of covariates increases, logistic regression and nn-1 have larger standard errors, while bagging, cart, dnn-2, dnn-3, and forest have significantly smaller standard errors, with cart and forest having the smallest standard errors (SE = 0.17 and 0.16, respectively).

To supplement the results, I also evaluated the mean squared error (MSE), which summarizes the bias and variance of the ATE. MSE measures the average squared difference between the estimated values and the true values, where a lower MSE indicates a better fit between the two. The results of the simulation in terms of MSE can be found in \@fig-ate-var-2. In the low-dimensional scenario with 20 covariates, all methods perform similarly in terms of MSE. However, as the dimensionality increases, the DNN-2 approach performs the best, resulting in the lowest MSE value (MSE = 0.87), which is 35 times lower than the MSE for logistic regression (MSE = 29.4).

Finally, I evaluate how well each method captures the true ATE of 0.3 within its estimated 95% confidence interval. In simulation studies, the coverage rate of the 95% confidence interval is crucial to consider. It reflects the precision of the estimated ATE and the likelihood of capturing the true ATE within the interval. Low coverage implies a wide confidence interval, meaning the estimated ATE is imprecise and uncertain. On the other hand, high coverage indicates a narrow confidence interval, meaning the estimated ATE is more precise. Evaluating the coverage rate provides valuable insights into the consistency and reliability of the estimation methods used. \@fig-ate-var-3 shows the results of each method's 95% confidence interval coverage, with the dashed line indicating a 95% coverage rate. In the low-dimensional scenario, logistic regression and nn-1 yield better coverage rates compared to other methods. However, with high numbers of covariates, bagging has the best coverage rate, followed by logistic regression and the neural network approaches. Bagged cart and random forest produce low coverage rates across all covariate conditions.
...
As with the previous ATE bias results, I also examined each method based on the complexities in the treatment and outcome models. The upper left panel of \@fig-ste-var-cond presents the results in which both the treatment and outcome models include only main effects. I found that when the outcome and treatment models only include main effects, all methods produced low SE and MSE across covariate conditions. Additionally, bag, dnn-2, dnn-3, nn-1, and logit had satisfactory 95% confidence interval coverage, while both cart and forest had relatively low coverage rates.

#### Complex Treatment and Outcome Population Models

When both the treatment and outcome models include quadratic and interaction terms, the results vary. As the number of covariates increases to 100, logistic regression, cart, random forest, and nn-1 have large SE and MSE values. Logistic regression has the largest MSE value (MSE = 1.05). In the highest dimensional case of 200 covariates, logistic regression leads to extremely high SE and MSE values (SE = 3.24; MSE = 115.1) compared to all other machine learning approaches, except for nn-1. This is in contrast to the low SE and MSE values of logistic regression in the base condition (SE = 0.013; MSE = 0.00008). DNN-2 and DNN-3 have significantly lower MSE values compared to all other methods, with MSE values below 3, which is nearly 40 times lower than the logistic MSE value in the 200 covariate condition.

In terms of 95% confidence interval coverage, when complexities are in both the outcome and treatment models, the coverage rates decrease. Additionally, as the number of covariates increases, logit and nn-1 are sensitive to covariate number, while other machine learning approaches are robust to covariate balance variation. Overall, in the highest covariate condition, bagging and the DNN approaches have higher coverage than logistic regression, but it is not ideal. As in the overall findings, both cart and forest have very low coverage rates.

#### Deviations from the Treatment and Outcome Models

As with the results from the ATE bias evaluation, I find that the standard error (SE) and mean squared error (MSE) are sensitive to complexities in the outcome model, more so than when complexities are only included in the treatment model. When complexities are only included in the outcome model, bagging, DNN-2, DNN-3, and logistic regression are robust to covariate conditions, with MSE values below 1. In contrast, nn-1 has the highest MSE, followed by bagged cart and random forest. When complexities are only included in the outcome model, all methods are robust to covariate conditions, while when complexities are only in the treatment model, logistic regression and nn-1 become sensitive to covariate conditions, leading to worse coverage rates with more covariates.

These results imply that on average my DNN-2 approach is a reliable and consistent method for estimating ATE in high-dimensional contexts, as it has the lowest MSE and good 95% confidence interval coverage. Bagging also performed well in terms of 95% confidence interval coverage, making it a strong contender in high-dimensional contexts as well. Logistic regression and nn-1 had better coverage rates in low-dimensional contexts, but their performance decreases as the number of covariates increases. The results suggest that it is important to consider both the bias and variability of the estimated ATE when choosing the optimal estimation method.
...
Covariate balance is a critical aspect to consider in simulation studies that use propensity score weighting. The average standardized absolute mean difference (ASAM) between the treatment and control groups is a metric used to assess balance. The ASAM reflects the degree of similarity in the covariate distributions between the two groups, which can significantly impact the simulation results. Maintaining covariate balance reduces the risk of bias in the estimated treatment effects and leads to more accurate results. An ASAM of 0 indicates balance between treated and untreated units, on average across all covariates. ASAM is also important because in practice, covariate balance is the only aspect that can be observed and used to make decisions on which method to use for propensity score estimation. However, whether ASAM is the right metric to assess balance is contested, with some researchers suggesting that other moments besides the mean should be considered. For my simulation I exclusively focus on accessing balance through the mean.

The results of the covariate balance evaluation can be seen in \@fig-asam. Across all covariate levels, the ASAM values were acceptable for all methods using the common 0.2 threshold (CITE), except for CART, Random Forest, and Logistic Regression, which had the largest ASAM values. Using the more stringent 0.1 threshold proposed by CITE, bagging, DNN-2, and DNN-3 performed better. In the highest covariate case, logistic regression produced extreme imbalance (ASAM = 74.9), while all other machine learning approaches produced good covariate balance with ASAM values below 0.15. The DNN-2 approach produced the best balance with an ASAM value of 0.06.
...
When both the outcome and treatment models included only main effects, all methods produced good covariate balance using the 0.2 threshold across all covariate conditions (see Figure x). With the more stringent 0.1 threshold, bagging, the neural network-based approaches, and logistic regression produced optimal balance.
...
I evaluated covariate balance when I introduced complexities to the treatment and outcome model. In line with the overall ASAM results, logistic regression and nn-1 produced extreme imbalances among covariates in the higher covariate condition. For example, with 200 covariates, logistic regression produced extreme imbalances (ASAM = 74.94). In comparison, my DNN-2 approach achieved good covariate balance (ASAM = 0.061). These results suggest that when complexities are included in the outcome and treatment model, my DNN approach will lead to the best covariate balance among all other methods, especially in high-dimensional cases.

#### Deviations from the Treatment and Outcome Models

I found that covariate balance is fairly robust to different specifications of covariate conditions and complexities in the outcome and treatment models. However, when dealing with higher covariates, specifically 100 and 200, I observed that logistic and nn-1 are sensitive to complexities in the treatment models.

I evaluated covariate balance in simulation studies that use propensity score weighting. The average standardized absolute mean difference (ASAM) between the treatment and control groups was used as the metric to assess balance. The results showed that across all covariate levels, most methods produced acceptable balance using a 0.2 threshold, except for CART, Random Forest, and Logistic Regression. With a more stringent 0.1 threshold, bagging, DNN-2, and DNN-3 performed better. In the highest covariate case, logistic regression produced extreme imbalance, while the DNN-2 approach produced the best balance with an ASAM value of 0.06. The results also showed that when both the outcome and treatment models included complexities, the DNN-2 approach led to the best covariate balance among all other methods, especially in high-dimensional cases. However, logistic and nn-1 were sensitive to complexities in the treatment models in high-dimensional settings. This highlights the importance of considering complexities in the treatment models when evaluating covariate balance.

### Weights Assessment

In my simulation study, I estimated the average treatment effect (ATE) using inverse propensity score weighting (IPTW). I found that assessing the mean of the IPTW is important in evaluating the balance achieved by the propensity score weighting method. The mean of the IPTW is a measure of the overall balance between the treatment and control groups, and a value far from 1 may indicate unbalanced covariate distributions, leading to biased treatment effect estimates. It is important to ensure that the mean of the IPTW is close to 1 to reduce the risk of bias in the estimated ATE.

@fig-weights showe the results for mean IPTW weights across both treated and untreated units. Note that by contructions all weights will have a value above 1. All machine learnig methods lead to acceptable weights below 2.5. However, in the high dimensional cases logisti regression and nn-1 is producing extreme weights. In the 200 covariate case logistic regression produces extreme weights (IPTW = 2.258688e+08), as does nn-1 (IPTW = 1.304062e+05)
...
When only main effects were present in the outcome and treatment model, I found that all methods produced acceptable inverse propensity score weights (IPTW) with mean values below 2. The mean values of the IPTW were consistent across all covariate conditions, indicating that the balance between the treatment and control groups was not affected by the presence of covariates. This suggests that all methods were able to produce balanced treatment and control groups, leading to less biased treatment effect estimates.
...
I found that when I introduced complexities in both the treatment and outcome models, the results were similar to the overall results. All machine learning methods produced acceptable IPTW weights below 2.5, except for logistic regression and nn-1 in high-dimensional cases, which produced extreme weights.

#### Deviations from the Treatment and Outcome Models

I find that inverse propensity score weights (IPTW) were fairly robust to complexities in the treatment and outcome model. However, in the higher covariate conditions, logistic and nn-1 produced extreme IPTW weights. This indicates that these methods may not be suitable for high-dimensional settings where the treatment and outcome models have complex relationships. It is important to consider the complexities in the treatment and outcome models when selecting a method for IPTW weighting to ensure that the weights are not extreme and produce accurate treatment effect estimates.

The results showed that, in general, all machine learning methods produced acceptable IPTW weights below 2.5, except for logistic regression and nn-1 in high-dimensional cases with complex treatment and outcome models. When only main effects were present, all methods produced acceptable IPTW weights with mean values below 2. The robustness of IPTW weights to complexities in the treatment and outcome model highlights the importance of considering these complexities when selecting a method for IPTW weighting to ensure accurate treatment effect estimates.

### Power

Statistical power refers to the probability of correctly detecting a true difference in treatment effects at a specified significance level, usually .05. Low power means that the study is more likely to have a high rate of type II errors, meaning that it may fail to detect real differences in treatment effects. On the other hand, high power indicates a low rate of type II errors, which increases the accuracy and reliability of the results. However, it's important to keep in mind that statistical power can be influenced by factors such as sample size and effect size, and may not always be the best metric to analyze in simulation studies. Particularly in this simulation where sample size was fixed to 10,000 and the treatment effect was relatively small. In what follows I present a brief summary of my results.

The results of the statistical power evaluation can be seen in \@fig-power. The standard alpha level of 0.5 was used to assess statistical significance. With 20 covariates, all methods were able to correctly identify a statistically significant treatment effect 100% of the time. However, as the number of covariates increased, a decrease in power was observed.
...
-complexities in outcome effect bias more. simulaiton should take this into account

I. Introduction

-   Brief overview of the purpose and main findings of the study

II\. Summary of Main Results

-   Summary of results for the ATE bias, standard error, mean squared error, and 95% confidence interval coverage

-   Comparison of the performance of each method across different covariate conditions and treatment and outcome complexities

-   Discussion of the results of the covariate balance evaluation using the ASAM metric

III\. Implications and Contributions of the Study

-   Discussion of the implications of the results for practice and future research
...
-   Discussion of the limitations of the study and potential sources of bias or confounding
...
-   Final thoughts and recommendations for practitioners and researchers in the field.Tables and Figures
